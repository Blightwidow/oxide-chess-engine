use crate::defs::{file_of, rank_of, square_of, type_of_piece, PieceType, Square};
use crate::movegen::defs::{Move, MoveTypes};
...
/// Parse an EPD line like:
/// `r1bqk1r1/... w q - - bm Rxf4; id "ERET 001 - Relief";`
fn parse_epd(line: &str) -> Option<ParsedEpd> {
    let line = line.trim();
    if line.is_empty() {
        return None;
    }

    // Split into tokens; FEN is the first 4 fields (board, side, castling, ep)
    let parts: Vec<&str> = line.splitn(5, ' ').collect();
    if parts.len() < 5 {
        return None;
    }
    let fen = format!("{} {} {} {}", parts[0], parts[1], parts[2], parts[3]);

    // The rest contains operations like "bm Rxf4; id "ERET 001";"
    let operations = parts[4];

    // Determine bm or am
    let avoid;
    let moves_str;
    if let Some(rest) = operations
        .strip_prefix("- bm ")
        .or_else(|| operations.strip_prefix("bm "))
    {
        avoid = false;
        moves_str = rest;
    } else if let Some(rest) = operations
        .strip_prefix("- am ")
        .or_else(|| operations.strip_prefix("am "))
    {
        avoid = true;
        moves_str = rest;
    } else {
    ...
    // Extract moves (everything before the first ';')
    let moves_end = moves_str.find(';').unwrap_or(moves_str.len());
    let expected_moves: Vec<String> = moves_str[..moves_end]
        .split_whitespace()
        .map(|s| s.trim_end_matches(',').to_string())
        .collect();

    // Extract id
    let id = if let Some(id_start) = moves_str.find("id \"") {
        let after = &moves_str[id_start + 4..];
...
/// Convert a SAN move string (e.g. "Rxf4", "Ng6", "dxe5", "e8N") to a Move,
/// given the list of legal moves and the current position.
fn san_to_move(san: &str, legal_moves: &[Move], position: &Position) -> Option<Move> {
    ...
    if san == "O-O" || san == "O-O-O" {
        ...
        for &mv in legal_moves {
            if mv.type_of() == MoveTypes::CASTLING {
                let from_file = file_of(mv.from_sq());
                let to_file = file_of(mv.to_sq());
                if kingside && to_file > from_file {
                    return Some(mv);
                }
                if !kingside && to_file < from_file {
                    return Some(mv);
    ...
    // Parse disambiguation and target square
    let mut disambiguation_file: Option<usize> = None;
    let mut disambiguation_rank: Option<usize> = None;
    let mut target_file: Option<usize> = None;
    let mut target_rank: Option<usize> = None;
    let mut promotion_type = PieceType::NONE;

    // Skip 'x' for captures
    let remaining: String = san[index..].chars().filter(|c| *c != 'x').collect();
    let remaining_bytes = remaining.as_bytes();

    // Parse promotion: look for =N, =B, =R, =Q or trailing N/B/R/Q for pawns
    let mut end = remaining_bytes.len();
    if end >= 2 && remaining_bytes[end - 2] == b'=' {
        promotion_type = match remaining_bytes[end - 1] {
    ...
    } else if piece_type == PieceType::PAWN && end >= 3 {
        // Handle "e8N" style promotion (no = sign)
        match remaining_bytes[end - 1] {
            b'N' => {
                promotion_type = PieceType::KNIGHT;
                end -= 1;
            }
            b'B' if end >= 2 && remaining_bytes[end - 2].is_ascii_digit() => {
                // Ambiguous: could be file 'b' + rank, or promotion to bishop
                // If the char before is a digit (rank 8 or 1), it's a promotion
                let rank = (remaining_bytes[end - 2] - b'1') as usize;
                if rank == 0 || rank == 7 {
    ...
    // The last two chars should be the target square (file + rank)
    if end >= 2 {
        let file_char = remaining_bytes[end - 2];
        let rank_char = remaining_bytes[end - 1];
        if (b'a'..=b'h').contains(&file_char) && (b'1'..=b'8').contains(&rank_char) {
            target_file = Some((file_char - b'a') as usize);
            target_rank = Some((rank_char - b'1') as usize);
        }
    }

    // Anything before the target square is disambiguation
    let disambig_end = end.saturating_sub(2);
    for &ch in &remaining_bytes[..disambig_end] {
        if (b'a'..=b'h').contains(&ch) {
            disambiguation_file = Some((ch - b'a') as usize);
        } else if (b'1'..=b'8').contains(&ch) {
            disambiguation_rank = Some((ch - b'1') as usize);
        }
    }

    let target_sq = match (target_file, target_rank) {
        (Some(file), Some(rank)) => square_of(file, rank),
        _ => return None,
    };
...
fn move_matches(
    ...
    piece_type: usize,
    target_sq: Square,
    disambiguation_file: Option<usize>,
    disambiguation_rank: Option<usize>,
    promotion_type: usize,
    position: &Position,
) -> bool {
    let to = mv.to_sq();
    let from = mv.from_sq();

    // Castling handled separately in san_to_move
    if mv.type_of() == MoveTypes::CASTLING {
        return false;
    }
    ...
    let moving_piece = type_of_piece(position.piece_on(from));
    if moving_piece != piece_type {
        return false;
    }

    if let Some(file) = disambiguation_file {
        if file_of(from) != file {
            return false;
        }
    }
    if let Some(rank) = disambiguation_rank {
        if rank_of(from) != rank {
            return false;
        }
    }

    if promotion_type != PieceType::NONE {
        if mv.promotion_type() != promotion_type {
            return false;
        }
    } else if mv.promotion_type() != PieceType::NONE {
        return false;
    }
...
pub fn run_eret(search: &mut Search, limit: EretLimit) {
    ...
    for position in &positions {
        ...
        search.position.set(position.fen.clone());
        ...
        let ok = if let Some(mv) = result {
            ...
            let matches = position.expected_moves.iter().any(|san| {
                san_to_move(san, &legal_moves, &search.position)
                    .map(|expected| format!("{:?}", expected) == mv_str)
                    .unwrap_or(false)
            });
            if position.avoid {
                !matches
        ...
        let mv_display = result
            .map(|mv| format!("{:?}", mv))
            .unwrap_or_else(|| "none".to_string());
        let expected_display = if position.avoid {
            format!("am {}", position.expected_moves.join(", "))
        } else {
            position.expected_moves.join(", ")
        };

        println!(
            "{}: {} — played {} (expected {}) [{} nodes]",
            position.id,
    ...
    println!("Total time (s): {:.1}", duration.as_secs_f64());
    println!("Nodes searched: {}", total_nodes);
    println!("Nodes/second  : {}", 1000 * total_nodes / duration.as_millis() as usize);
